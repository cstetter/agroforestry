...
colnames(mat_s) <- c("Scenario", 
                     rep(c("MC", "MC V", "MU L", "PE S", "Gre-en"), 3))
```
...
tab <- kbl(mat_s, booktabs = T, format = "latex", 
 ...
 footnote(general_title = "", footnote_as_chunk = T,
          general = "useful lifetime (years), PES = Payments for environmental services (Euro), Green = Cultivated") %>%
...
# Load models
```{r}
file_list <- list.files("models", full.names = T)[-1]

res_list <- lapply(file_list, function(i) readRDS(file = i))
names(res_list) <- stringr::str_sub(
 list.files("models", full.names = F)[-1], 1,-5)
...
res_cor_base_13_410_vcov <- 
 summary(vcov(res_cor_base_13_410, what = "rpar", type = "cor"))
...
res_cor_base_13_410_tidy <- rbind(
 tidy(res_cor_base_13_410),
 setNames(
  as.tibble(cbind(rownames(res_cor_base_13_410_vcov),
                  res_cor_base_13_410_vcov[,1:4])), 
  names(tidy(res_cor_base_13_410)))) %>%
 filter(substr(term,1,1) != "c")
...
broom_glance <- function(res_object, cor){
 glance(res_object)  %>%
  dplyr::select(logLik, rho2, AIC, nobs)
}

res_mnl_13_410_glance <- broom_glance(res_object = res_mnl_13_410)
res_cor_base_13_410_glance <- broom_glance(res_object = res_cor_base_13_410)
...
model_out <- texreg(
 ...
 custom.gof.rows = list("Correlation"=c("-", "No", "Yes"),
                        " " = c(" ", " ",  " ")),
 custom.gof.names = c(NA, "Pseudo-R2", NA, "Obs."),
 ...
 custom.model.names = c("MNL", "uncor. RPL", "cor. RPL"),
...
```{r}
rob_list <- res_list[grepl("CR25", names(res_list))]
...
names_lags <- 
 c("\\textbf{Selected Model} short-term: 1-3 years,\nlong-term: 4-10 years",
   ...
   "\\textbf{Specification Alt. 4} short-term: 1-3 year, long-term: 4-15 years",
   "\\textbf{Specification Alt. 5} short-term: 1-3 year, long-term: 4-20 years",
   "\\textbf{Specification Alt. 6} short-term: 1-5 year, long-term: 6-10 years",
   "\\textbf{Specification Alt. 7} short-term: 1-5 year, long-term: 6-15 years",
   "\\textbf{Specification Alt. 8} short-term: 1-5 year, long-term: 6-20 years")
...
row.names(tab_descriptive) <-c("Full-time farming (1 if yes, 0 otherwise)",
                               ...
                               "Full-time farming (1 if yes, 0 otherwise)",
                               "Farmer's age (years)",
